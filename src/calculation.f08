...
contains
    ...
    function get_spectre_by_z(z, spectres) result(spec)
        ...
        do i = 1, length - 1
            ...
            ! print *, diff
            if ( spectres(i)%redshift <= z .and. z <= spectres(i+1)%redshift ) then
                spec = approximate_spectres(spectres(i), spectres(i+1), z)
                ! write(*,*) i, "___", i+1
    ...
    function approximate_spectres(a, b, z) result(spec_result)
      implicit none
      type(spectre), intent(in) :: a, b
      real*8, intent(in) :: z
      real*8 :: X_a, X_b
      real*8, dimension(:), allocatable :: spec_result, Y_a, Y_b
      integer :: i
      ...
      spec_result = Y_a + (Y_b - Y_a) * ((z - X_a)/(X_b - X_a))

      ! open(unit=432, file="approximate.out", status="unknown", action="write")
      ! do i = 1, size(spec_result)
      !   write(*, "(3(E12.3E2))")  Y_a(i), spec_result(i), Y_b(i)
      ! end do
      ! write (*,*) size(spec_result)
      ! close(432)

    end function approximate_spectres
    ...
    function integrate_gamma(energy,spectres,kind,z) result(sum)
        ...
        do i = size(energy), 1, -1
            ...
            if (e_curr > E_0) then
              ...
              if (size(spectre_z) .ne. size(energy)) then
